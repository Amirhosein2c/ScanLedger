...
export const useGoogleOAuth = ({
  onSuccess,
  onError,
}: UseGoogleOAuthParams = {}): UseGoogleOAuthResult => {
  const [isReady, setIsReady] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const credentialCallbackRef = useRef<
    (response: GoogleCredentialResponse) => Promise<void> | void
  >(() => {});
  ...
  useEffect(() => {
    credentialCallbackRef.current = async (
  ...
  useEffect(() => {
    ...
    if (!isValidClientId(GOOGLE_CLIENT_ID)) {
      const invalidClientError = new Error(
        "Missing or invalid Google Client ID"
      );
  ...
  const triggerSignIn = useCallback(() => {
    ...
    window.google.accounts.id.prompt((notification) => {
      ...
      if (notification.isNotDisplayed()) {
        ...
        const err = new Error(
          `Google Sign-In could not be displayed (${reason})`
        );
        setError(err);
        onError?.(err);
        ...
        const err = new Error(
          `Google Sign-In was dismissed (${notification.getDismissedReason()})`
        );
        setError(err);
        onError?.(err);
      }
